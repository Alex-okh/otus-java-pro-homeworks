...
class DataTemplateHibernateTest extends AbstractHibernateTest {
    ...
    void shouldSaveAndFindCorrectClientById() {
        ...
        // when
        var clientList = transactionManager.doInReadOnlyTransaction(session -> clientTemplate.findAll(session)
                                                                                             .stream()
                                                                                             .map(Client::clone)
                                                                                             .toList());
        ...
        assertThat(clientList.getFirst()).usingRecursiveComparison()
                                         .isEqualTo(savedClient);

        // when
        clientList = transactionManager.doInReadOnlyTransaction(
                session -> clientTemplate.findByEntityField(session, "name", "updatedName")
                                         .stream()
                                         .map(Client::clone)
                                         .toList());
